...
Example of a model detail

![ml_flow_model](/assets/ml_flow_model.png)

An MLFlow model's **artifacts** are stored in GCS: 

![ml_flow_model_artifact_GCS_](/assets/ml_flow_model_artifact_GCS_.png)


These files are
  - `model.skops`: the actual serialized scikit-learn model, including preprocessing and logistic regression.
  - `MLmodel`: MLflow metadata describing the model format, flavor, signature, and artifact location.
  - `input_example.json`: example input used when logging the model.
  - `serving_input_example.json`: example request format for serving.
  - `requirements.txt`: Python dependencies needed to load/run the model.
  - `conda.yaml / python_env.yaml`: environment descriptions generated by MLflow.
